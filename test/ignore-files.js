...
function finish() {
  ...
  assert.ok(output[2].match(/dir2\/bleh$/));
  assert.ok(output[3].match(/dir2\/subdir$/));
  assert.ok(output[4].match(/dir2\/subdir\/weeee$/));
}
...
rs.pipe(dir({ ignoreExtensions: ['txt'] })).pipe(a);
